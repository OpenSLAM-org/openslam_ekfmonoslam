...
%   $Revision: 1.0
  ...
  if max(size(p0))==3
      if max(size(p1))==3
          ...
          p = p1-p0;
          alpha = 0.15;  % Size of arrow head relative to the length of the vector
          beta = 0.15;  % Width of the base of the arrow head relative to the length
          
%           hu = [x1-alpha*(p(1)+beta*(p(2)+eps)); x1; x1-alpha*(p(1)-beta*(p(2)+eps))];
%           hv = [y1-alpha*(p(2)-beta*(p(1)+eps)); y1; y1-alpha*(p(2)+beta*(p(1)+eps))];
%           hw = [z1-alpha*p(3);z1;z1-alpha*p(3)];
          
          hu = [x1-alpha*(p(1)+beta*(p(3)+eps)); x1; x1-alpha*(p(1)-beta*(p(3)+eps)); x1-alpha*(p(1)+beta*(p(3)+eps))];
          hv = [y1; y1; y1; y1];
          hw = [z1-alpha*(p(3)-beta*(p(1)+eps)); z1; z1-alpha*(p(3)+beta*(p(1)+eps)); z1-alpha*(p(3)-beta*(p(1)+eps))];
          
          hold on
          % plot3(hu(:),hv(:),hw(:),'color',color)  % Plot arrow head
          fill3(hu(:),hv(:),hw(:),color)  % Plot arrow head
          grid on
%           xlabel('x')
%           ylabel('y')
%           zlabel('z')
